...
export default function AdminEventOrderReception() {
    ...
    const [activeTab, setActiveTab] = useState<'전체' | '접수' | '배송'>('전체');
    ...
    const {data: orderHistoryData, refetch} = useQuery({
        queryKey: ["admin", "orders", "tst-order", "list", activeTab],
        queryFn: async (): Promise<any> => {
          const response = await api.post<any>("/api/orders/tst-order/list", {
            ord_sts_cd: activeTab === "전체" ? null : activeTab ==="접수" ? "1" : "2",
          });
            ...
            <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
                <div className="px-4 pb-4">
                    <div className="space-y-5">
                        {orderHistoryData?.orders?.map((item) => (
                            <div
                                key={item.ord_no}
                                ...
                                <div className='border border-border-default rounded-2xl p-4 pb-3'>
                                    <div className="flex justify-between items-center gap-1 mb-4">
                                        ...
                                        <button
                                            ...
                                            className={`h-7 px-3 py-1 rounded-sm text-xs font-bold leading-[16px] cursor-pointer ${item?.ord_sts_cd === '2'
                                                ? 'bg-[#28A745] text-white'
                                                : 'bg-white border border-action-primary text-black'
                                                }`}
                                        >
                                            {item?.ord_sts_cd === "1" ? "접수" : "배송"}
                                        </button>
                                    </div>
